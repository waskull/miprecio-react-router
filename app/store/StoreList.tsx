...
export default function StoreList({ data }: { data: ICompanyStore[] }): JSX.Element {
    ...
    const style: string = "flex items-center bg-zinc-50 rounded-lg cursor-pointer min-w-md shadow sm:flex dark:bg-gray-800";
    return (

        <section className="bg-white dark:bg-gray-900">
            <div className="py-8 px-4 mx-auto max-w-screen-lg lg:py-12 lg:px-4 ">
                <div className="mx-auto max-w-screen-sm text-center mb-8 lg:mb-8">
                    <h2 className="mb-4 text-4xl tracking-tight font-extrabold text-gray-900 dark:text-white">Nuestros Socios</h2>
                    <p className="font-light text-gray-500 lg:mb-12 sm:text-xl dark:text-gray-400">Selecciona uno de nuestros socios para ver sus productos</p>
                </div>
                <div className="grid gap-8 mb-6 lg:mb-16 md:grid-cols-2 sm:grid-cols-1">

                    {data.map((store: ICompanyStore) => (
                        <Card onClick={() => { setSelected(store.uid) }} className={selected === store.uid ? style+" ring-1 border-zinc-700 dark:ring-2 dark:ring-zinc-300" : style } key={store.uid} imgSrc="/sofia.png" horizontal>
